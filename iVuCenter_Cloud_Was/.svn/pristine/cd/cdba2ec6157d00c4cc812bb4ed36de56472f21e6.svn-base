...
public class UpdateBillChargeDiskResourceForAmountWriter implements ItemWriter<BatchBillChargeResourceVO> {

	private final String batchBillingDAO = "com.ivucenter.cloud.batch.billing.BatchBillingDAO.";
	
	private final String updateBillChargeDiskResourceForAmount = batchBillingDAO + "updateBillChargeDiskResourceForAmount";
...
//        this.baseBillEndDate = (Date) this.parameterValues.get("baseBillEndDate");
        ...
        for (BatchBillChargeResourceVO item : items) {
        	this.sqlSessionTemplate.update(updateBillChargeDiskResourceForAmount, item);
        }		
	}
