...
(/** @lends <global> */function( $ ) {
	...
	function _fnLanguageCompat( oLanguage )
	{
		...
		if ( !oLanguage.sEmptyTable && zeroRecords &&
			oDefaults.sEmptyTable === "데이터가 존재하지 않습니다." )
		{
	...
	function _fnBuildAjax( oSettings, data, fn )
	{
		...
		var baseAjax = {
			...
			"error": function (xhr, error, thrown) {
				...
				if ( error == "parsererror" ) {
					alertBox("접속이 종료되어 로그인화면으로 이동됩니다.<br>다시 로그인해주십시오.", logout);
				}else if( error == "error"){
					if(xhr.status == 901){
						alertBox("동일한 ID로 접속정보가 확인되어<br>로그인화면으로 이동됩니다.<br>다시 로그인해주십시오.", logout);
					}
