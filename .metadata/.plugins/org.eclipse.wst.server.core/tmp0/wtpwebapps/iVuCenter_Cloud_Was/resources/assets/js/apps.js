    ...
    $('.sidebar .nav > .has-sub > a').click(function() {
        ...
        if ($('.page-sidebar-minified').length === 0) {
            $(otherMenu).not(target).slideUp(250, function() {
                $(this).closest('li').removeClass('expand111');
            });
            $(target).slideToggle(250, function() {
                var targetLi = $(this).closest('li');
                if ($(targetLi).hasClass('expand111')) {
                    $(targetLi).removeClass('expand111');
                } else {
                    $(targetLi).addClass('expand111');
                }
            });
        }*/
    	$(this).parent().children().next().slideToggle().parent().toggleClass("active");
    	$(this).parent().siblings().children().next().slideUp();
    	$(this).parent().siblings().removeClass("active");
    });
    $(document).ready(function(){
    	if($('.nav > li.has-sub').hasClass("active")){
    		//$('.nav > li.active').addClass("expand");
    	};
    });
...
var handlePanelAction = function() {
    ...
    $(document).on('hover', '[data-click=panel-collapse]', function() {
        $(this).tooltip({
         /*   title: 'Collapse / Expand',  */    /*     title: 'Collapse / Expand',   > 접기 /펼치기  */
            placement: 'bottom',
            trigger: 'hover',
            container: 'body'
        });
