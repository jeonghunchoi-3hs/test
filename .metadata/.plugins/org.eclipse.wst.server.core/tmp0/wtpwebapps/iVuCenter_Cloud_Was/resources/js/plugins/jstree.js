 ...
 */
	...
	$.jstree.defaults.checkbox = {
		 ...
		 * 2014 07 01 정승탁 추가
		 * 
		 * 체크박스 플러그인에서 오직 체크박스만 체크햇을때 이벤트 발생 기본 : true , 체크 박스만 원하면 false로
		 */
		only_checkbox 		: true
	};
	$.jstree.plugins.checkbox = function (options, parent) {
		...
		this.activate_node = function (obj, e) {
			...
			//2014 07 01 정승탁 추가   - row전체에 checkbox이벤트 발생 (jstree default)
			if(this.settings.checkbox.only_checkbox){
				return parent.activate_node.call(this, obj, e.target);
			}else{		//only_checkbox false일때 작동하며, customizing 함
				if($(e.target).hasClass('jstree-checkbox')){	//체크박스를 클릭할경우 기존 로직과 동일하게 처리
					return parent.activate_node.call(this, obj, e);
				}else{											//그외에는 chaned trigger를 강제적으로 발생
					var nodeId = this.get_node($(obj).parent('li').attr('id'));
					this.trigger('changed', { 'action' : 'select_node', 'node' : obj, 'selected' : this._data.core.selected, 'event' : e });
					//return  parent.select_node(obj, false, false);
					return;
				}
			}
		};
	...
	$.jstree.plugins.contextmenu = function (options, parent) {
		...
		this.show_contextmenu = function (obj, x, y, e) {
			...
			if(this.settings.contextmenu.select_node && !this.is_selected(obj)) {
				//수정 context메뉴시 select되면서 굳이 필요없는 로직을 함 -정승탁
				//this.deselect_all();
				//this.select_node(obj, false, false, e);
			}
