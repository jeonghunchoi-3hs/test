...
public class InsertBillInvoiceProjectDetailForCloudProductWriter implements ItemWriter<BatchBillInvoiceProjectDetailVO> {
...
//        this.baseBillEndDate = (Date) this.parameterValues.get("baseBillEndDate");
        ...
        for (BatchBillInvoiceProjectDetailVO item : items) {
			item.setInvoiceItem(InvoiceTemplate.CLOUD_PRODUCT.getCode());
			item.setInvoiceItemSub("*");
			item.setInvoiceItemName(InvoiceTemplate.CLOUD_PRODUCT.getLabel());
			item.setInvoiceItemDepth(InvoiceTemplate.CLOUD_PRODUCT.getDepth());
