...
public class InsertBillInvoiceProjectDetailForBasicProductWriter implements ItemWriter<BatchBillInvoiceProjectDetailVO> {
...
//        this.baseBillEndDate = (Date) this.parameterValues.get("baseBillEndDate");
        ...
        for (BatchBillInvoiceProjectDetailVO item : items) {
			item.setInvoiceItem(InvoiceTemplate.BASIC_PRODUCT.getCode());
			item.setInvoiceItemSub("*");
			item.setInvoiceItemName(InvoiceTemplate.BASIC_PRODUCT.getLabel());
			item.setInvoiceItemDepth(InvoiceTemplate.BASIC_PRODUCT.getDepth());
