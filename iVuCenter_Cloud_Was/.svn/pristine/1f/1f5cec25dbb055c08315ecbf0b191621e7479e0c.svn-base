...
public class InsertBillChargeDiskResourceWriter implements ItemWriter<BatchBillChargeResourceVO> {
	
	private final String batchBillingDAO = "com.ivucenter.cloud.batch.billing.BatchBillingDAO.";
	
	private final String insertBillChargeDiskResource 	= batchBillingDAO + "insertBillChargeDiskResource";
...
//        this.baseBillEndDate = (Date) this.parameterValues.get("baseBillEndDate");
        ...
        for (BatchBillChargeResourceVO item : items) {
        	item.setResourceItem("volume.size");
        	
        	this.sqlSessionTemplate.insert(insertBillChargeDiskResource, item);
        }
	}
