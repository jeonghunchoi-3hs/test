...
public class ResourcesAvailReader implements ItemReader<OpenStackUsage>{

	private final String batchDeleteOssAvailabilityHistory = "com.ivucenter.cloud.portal.oss.availability.OssAvailabilityDAO.batchDeleteOssAvailabilityHistory";
		...
		for (CustomOssCloudVO cloudVo : lists) {
			if(cloudVo.getCloudType().equals("openstack")) {
				...
				if (this.openStackUsage.size() > 0) {
					this.sqlSessionTemplate.delete(batchDeleteOssAvailabilityHistory, cloudVo.getCloudId());
				}
