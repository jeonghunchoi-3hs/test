...
public class InsertBillChargeServiceResourceWriter implements ItemWriter<BatchBillChargeResourceVO> {

	private final String batchBillingDAO = "com.ivucenter.cloud.batch.billing.BatchBillingDAO.";
	
	private final String insertBillChargeServiceResource 	= batchBillingDAO + "insertBillChargeServiceResource";
...
//        this.baseBillEndDate = (Date) this.parameterValues.get("baseBillEndDate");
        ...
        for (BatchBillChargeResourceVO item : items) {
        	item.setResourceItem("EA");
        	
        	this.sqlSessionTemplate.insert(insertBillChargeServiceResource, item);
        }
	}
