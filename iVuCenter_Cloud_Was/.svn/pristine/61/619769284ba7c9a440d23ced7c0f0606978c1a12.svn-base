...
public class SendStatusJobLauncherDetails extends QuartzJobBean{
	private static final String SPRING_BATCH_JOB_NAME = "sendStatusJob";
    
    @Override
    protected void executeInternal(JobExecutionContext context){
        
    	log.debug("---------------------SendStatusJobLauncherDetails------------------------"); 
        String jobName = context.getJobDetail().getKey().getName();
        String current = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss").format(new Date());
        log.debug("[" + current + "] " + "Quartz job started: "+ jobName);
